...
async function generatePostmanCollection() {
  ...
  const moduleFolders = pluginModules.map(module => createModuleFolder(module));

  // Create Postman collection
  const collection = {
    info: {
      name: 'OPNsense Plugin Modules API',
      description: 'Complete API collection for OPNsense Plugin Modules',
  ...
  const totalEndpoints = pluginModules.reduce((sum, module) => sum + module.endpoints.length, 0);
  collection.info.description += `\n\nStatistics:\n- Plugin Modules: ${pluginModules.length}\n- Total Endpoints: ${totalEndpoints}`;

  // Write collection to file (updated path)
  const outputPath = path.join(__dirname, '../postman/OPNsense_Plugin_Modules_API_Collection.postman_collection.json');
